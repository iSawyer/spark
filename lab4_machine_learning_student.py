...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    num = len(IDandRatingsTuple[1])
    total_rating = 0.0
    for rating in IDandRatingsTuple[1]:
        total_rating += rating 
    return (IDandRatingsTuple[0], (num,total_rating/num))
...
trainingAvgRating = trainingRDD.map(lambda (a, b, c): c).sum() / trainingRDD.count()
...
myRatedMovies = [
      (myUserID, 318, 5), (myUserID, 858, 5), (myUserID, 1221, 5), (myUserID, 2571, 3), (myUserID, 3114, 4),
    (myUserID, 110, 5), (myUserID, 1036, 5), (myUserID, 364, 5), (myUserID, 1035, 3), (myUserID, 3793, 5), (myUserID, 1721, 5)
...
myUnratedMoviesRDD = (moviesRDD.map(lambda (a, b): (myUserID, a))
                      .subtract(myRatingsRDD.map(lambda (a, b, c): (a, b))))
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda x: (x[0],x[1][0])).filter(lambda x: x[1] > 75)

# Transform predictedRatingsRDD into an RDD with entries that are pairs of the form (Movie ID, Predicted Rating)
predictedRDD = predictedRatingsRDD.map(lambda (a,b,c):(b,c))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .join(moviesRDD).map(lambda (a, (b, c)): (b[0], c, b[1])))
