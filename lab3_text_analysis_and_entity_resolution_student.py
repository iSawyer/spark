...
def simpleTokenize(string):
    ...
    # res = []
    r = []
    res = re.split(split_regex,string)
    for item in res:
        if item == '':
            continue
        r.append(item)
...
# TODO: Replace <FILL IN> with appropriate code
corpusRDD = googleRecToToken + amazonRecToToken
...
def idfs(corpus):
    ...
    tokenCountPairTuple = uniqueTokens.flatMap(lambda x: [(key,1) for key in x]).reduceByKey(lambda x,y: x+y)
    tokenSumPairTuple = tokenCountPairTuple.map(lambda x: (x[0], 1.0*N/x[1]))
...
def tfidf(tokens, idfs):
    ...
    for item in tokens:
            ...
            tfIdfDict[item] = idfs[item] * tfs[item]
...
# TODO: Replace <FILL IN> with appropriate code
sims = similaritiesBroadcast.map(lambda (gurl, aid, score):(aid + ' ' + gurl, score))

trueDupsRDD = sims.join(goldStandard).map(lambda (k,v): (k,v[0]))

trueDupsCount = trueDupsRDD.count()
avgSimDups = trueDupsRDD.map(lambda (k,v): v).reduce(lambda x,y:x+y)/ float(trueDupsCount)
...
# TODO: Replace <FILL IN> with appropriate code
fullCorpusRDD = sc.union([googleFullRecToToken,amazonFullRecToToken])
...
def invert(record):
    ...
    """
    pairs = [(key,record[0]) for key in record[1].keys()]
    return (pairs)
