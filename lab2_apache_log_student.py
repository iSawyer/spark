...
# This was originally '^(\S+) (\S+) (\S+) \[([\w:/]+\s[+\-]\d{4})\] "(\S+) (\s*\S+)\s*(\S*\s*)" (\d{3}) (\S+)'
# 127.0.0.1 - - [01/Aug/1995:00:00:01 -0400] "GET /images/launch-logo.gif HTTP/1.0" 200 1839
...
uniqueHosts = hosts.reduceByKey(lambda x,y: 1)
...
dayHostCount = dayGroupedHosts.map(lambda x: (x[0], len(set(x[1])))).sortByKey()
...
badRecords = (access_logs
              .filter(lambda x: x.response_code == 404)).cache()
